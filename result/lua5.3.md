...
values with a C function, first these values must be pushed onto the
...
string. It is similar to the ISO C function `sprintf`, but has some
...
-   The conversion specifiers are quite restricted. There are no flags,
    ...
    (inserts the character '`%`'), '`%s`' (inserts a zero-terminated
    string, with no size restrictions), '`%f`' (inserts a
    [`lua_Number`](#lua_Number)), '`%I`' (inserts a
    [`lua_Integer`](#lua_Integer)), '`%p`' (inserts a pointer as a
    hexadecimal numeral), '`%d`' (inserts an `int`), '`%c`' (inserts an
    `int` as a one-byte character), and '`%U`' (inserts a `long int` as
...
given index. The access is raw, that is, it does not invoke the
`__index` metamethod.
...
index, `p` is encoded as a light userdata, and `v` is the value at the
top of the stack.

This function pops the value from the stack. The assignment is raw, that
is, it does not invoke `__newindex` metamethod.
...
API over it. The error object is on the top of the stack.
...
may trigger a metamethod for the "newindex" event (see [§2.4](#2.4)).
...
The status can be 0 ([`LUA_OK`](#pdf-LUA_OK)) for a normal thread, an
error code if the thread finished the execution of a
...
according to the lexical conventions of Lua (see [§3.1](#3.1)). The
string may have leading and trailing spaces and a sign. If the string is
not a valid numeral, returns 0 and pushes nothing. (Note that the result
...
`lua_tolstring` returns a pointer to a string inside the Lua state. This
...
`lua.h`: [`LUA_TNIL`]{#pdf-LUA_TNIL} (0),
[`LUA_TNUMBER`]{#pdf-LUA_TNUMBER}, [`LUA_TBOOLEAN`]{#pdf-LUA_TBOOLEAN},
[`LUA_TSTRING`]{#pdf-LUA_TSTRING}, [`LUA_TTABLE`]{#pdf-LUA_TTABLE},
...
in the Lua core. When called with a valid [`lua_State`](#lua_State),
returns the address of the version used to create that state. When
called with `NULL`, returns the address of the version running the call.
...
no continuation (see [§4.7](#4.7)). Therefore, when the thread resumes,
...
(see [§4.7](#4.7)). This continuation function receives the same stack
...
inside a line or a count hook (see [§4.9](#4.9)). In that case,
...
other fields of [`lua_Debug`](#lua_Debug) with useful information, call
[`lua_getinfo`](#lua_getinfo).

The fields of [`lua_Debug`](#lua_Debug) have the following meaning:

-   **`source`:** the name of the chunk that created the function. If
    `source` starts with a '`@`', it means that the function was defined
    in a file where the file name follows the '`@`'. If `source` starts
    with a '`=`', the remainder of its contents describe the source in a
    user-dependent manner. Otherwise, the function was defined in a
    string where `source` is that string.
...
-   **`nparams`:** the number of fixed parameters of the function
...
pops the function from the top of the stack.) For instance, to know in
...
-   **'`L`':** pushes onto the stack a table whose indices are the
    ...
    comments.)
    If this option is given together with option '`f`', its table is
    pushed after the function.
...
must be at the top of the stack. In this case, only parameters of Lua
...
which do not count on the stack). When there are no errors,
[`lua_getstack`](#lua_getstack) returns 1; when called with a level
...
all upvalues. (For Lua functions, upvalues are the external local
variables that the function uses, and that are consequently included in
its closure.)
...
Parameters `ar` and `n` are as in function
[`lua_getlocal`](#lua_getlocal).
...
Parameters `funcindex` and `n` are as in function
[`lua_getupvalue`](#lua_getupvalue).
...
Adds to the buffer `B` (see [`luaL_Buffer`](#luaL_Buffer)) a string of
length `n` previously copied to the buffer area (see
...
-   Finish by calling `luaL_pushresultsize(&b, sz)`, where `sz` is the
    total size of the resulting string copied into that space.
...
buffer must be declared as a variable (see
[`luaL_Buffer`](#luaL_Buffer)).
...
userdata address (see [`lua_touserdata`](#lua_touserdata)).
...
Checks whether the core running the call, the core that created the Lua
state, and the code making the call are all using the same version of
Lua. Also checks whether the core running the call and the core that
created the Lua state are using the same address space.
...
index `obj` and returns the type of pushed value. If the object does not
...
registry (see [`luaL_newmetatable`](#luaL_newmetatable)) (**nil** if
there is no metatable associated with that name). Returns the type of
the pushed value.
...
equivalent to the '`#`' operator in Lua (see [§3.4.7](#3.4.7)). Raises
an error if the result of the operation is not an integer. (This case
only can happen through metamethods.)
...
This function returns the same results as [`lua_load`](#lua_load), but
it has an extra error code [`LUA_ERRFILE`]{#pdf-LUA_ERRFILE} for
file-related errors (e.g., it cannot open or read the file).
...
`[tname] = new table`, and returns 1. (The entry `__name` is used by
...
an allocator based on the standard C `realloc` function and then sets a
panic function (see [§4.6](#4.6)) that prints an error message to the
...
`func` with the state `L` and the argument index `arg` as parameters.
...
If the result is `NULL` (only possible when returning `d` and
`d == NULL`), its length is considered zero.
...
integer keys into table `t`, [`luaL_ref`](#luaL_ref) ensures the
uniqueness of the key it returns. You can retrieve an object referred by
reference `r` by calling `lua_rawgeti(L, t, r)`. Function
[`luaL_unref`](#luaL_unref) frees a reference and its associated object.
...
If `modname` is not already present in
[`package.loaded`](#pdf-package.loaded), calls function `openf` with
string `modname` as an argument and sets the call result in
...
When `nup` is not zero, all functions are created sharing `nup`
upvalues, which must be previously pushed on the stack on top of the
library table. These values are popped from the stack after the
registration.
...
contain other data after this initial structure. Field `f` points to the
corresponding C stream (or it can be `NULL` to indicate an incompletely
created handle). Field `closef` points to a Lua function that will be
...
either **true** (in case of success) or **nil** plus an error message
(in case of error). Once Lua calls this field, it changes the field
...
by the function. If `len` is not `NULL`, the function also sets `*len`
...
an implementation in C (e.g., [`table.sort`](#pdf-table.sort)).
...
library), [`luaopen_utf8`]{#pdf-luaopen_utf8} (for the UTF8 library),
...
### [`assert (v [, message])`]{#pdf-assert}

Calls [`error`](#pdf-error) if the value of its argument `v` is false
(i.e., **nil** or **false**); otherwise, returns all its arguments. In
case of error, `message` is the error object; when absent, it defaults
to "`assertion failed!`"

------------------------------------------------------------------------

### [`collectgarbage ([opt [, arg]])`]{#pdf-collectgarbage}
...
-   **"`restart`":** restarts automatic execution of the garbage
    collector.
-   **"`count`":** returns the total memory in use by Lua in Kbytes. The
    value has a fractional part, so that it multiplied by 1024 gives the
    exact number of bytes in use by Lua (except for overflows).
-   **"`step`":** performs a garbage-collection step. The step "size" is
    controlled by `arg`. With a zero value, the collector will perform
    one basic (indivisible) step. For non-zero values, the collector
    will perform as if that amount of memory (in KBytes) had been
    allocated by Lua. Returns **true** if the step finished a collection
    cycle.
...
### [`getmetatable (object)`]{#pdf-getmetatable}

If `object` does not have a metatable, returns **nil**. Otherwise, if
the object's metatable has a `__metatable` field, returns the associated
...
### [`load (chunk [, chunkname [, mode [, env]]])`]{#pdf-load}
...
global environment. Other upvalues are initialized with **nil**. (When
you load a main chunk, the resulting function will always have exactly
one upvalue, the `_ENV` variable (see [§2.2](#2.2)). However, when you
load a binary chunk created from a function (see
[`string.dump`](#pdf-string.dump)), the resulting function can have an
arbitrary number of upvalues.) All upvalues are fresh, that is, they are
not shared with any other function.

`chunkname` is used as the name of the chunk for error messages and
debug information (see [§4.9](#4.9)). When absent, it defaults to
...
### [`rawset (table, index, value)`]{#pdf-rawset}
...
### [`setmetatable (table, metatable)`]{#pdf-setmetatable}

Sets the metatable for the given table. (To change the metatable of
other types from Lua code, you must use the debug library
([§6.10](#6.10)).) If `metatable` is **nil**, removes the metatable of
the given table. If the original metatable has a `__metatable` field,
raises an error.
...
the lexical conventions of Lua (see [§3.1](#3.1)). (The string may have
...
and 36, inclusive. In bases above 10, the letter '`A`' (in either upper
or lower case) represents 10, '`B`' represents 11, and so forth, with
'`Z`' representing 35. If the string `e` is not a valid numeral in the
given base, the function returns **nil**.

------------------------------------------------------------------------

### [`tostring (v)`]{#pdf-tostring}
...
converted, use [`string.format`](#pdf-string.format).)
...
### [`coroutine.create (f)`]{#pdf-coroutine.create}
...
A running coroutine is yieldable if it is not the main thread and it is
not inside a non-yieldable C function.

------------------------------------------------------------------------

### [`coroutine.resume (co [, val1, ···])`]{#pdf-coroutine.resume}
...
### [`coroutine.status (co)`]{#pdf-coroutine.status}
...
### [`coroutine.wrap (f)`]{#pdf-coroutine.wrap}
...
[`LUA_CPATH_5_3`]{#pdf-LUA_CPATH_5_3}, or the environment variable
...
### [`package.loadlib (libname, funcname)`]{#pdf-package.loadlib}
...
environment variable [`LUA_PATH_5_3`]{#pdf-LUA_PATH_5_3} or the
...
Any "`;;`" in the value of the environment variable is replaced by the
default path.
...
### [`package.searchpath (name, path [, sep [, rep]])`]{#pdf-package.searchpath}
...
### [`string.byte (s [, i [, j]])`]{#pdf-string.byte}
...
(re)loaded, those upvalues receive fresh instances containing **nil**.
(You can use the debug library to serialize and reload the upvalues of a
function in a way adequate to your needs.)

------------------------------------------------------------------------

### [`string.find (s, pattern [, init [, plain]])`]{#pdf-string.find}

Looks for the first match of `pattern` (see [§6.4.1](#6.4.1)) in the
string `s`. If it finds a match, then `find` returns the indices of `s`
where this occurrence starts and ends; otherwise, it returns **nil**. A
...
with no characters in `pattern` being considered magic. Note that if
...
### [`string.format (formatstring, ···)`]{#pdf-string.format}
...
The `q` option formats a string between double quotes, using escape
sequences when necessary to ensure that it can safely be read back by
...
Options `A`, `a`, `E`, `e`, `f`, `G`, and `g` all expect a number as
argument. Options `c`, `d`, `i`, `o`, `u`, `X`, and `x` expect an
integer. When Lua is compiled with a C89 compiler, options `A` and `a`
...
converted to one following the same rules of
[`tostring`](#pdf-tostring). If the option has any modifier (flags,
width, length), the string argument should not contain embedded zeros.

------------------------------------------------------------------------

### [`string.gmatch (s, pattern)`]{#pdf-string.gmatch}

Returns an iterator function that, each time it is called, returns the
next captures from `pattern` (see [§6.4.1](#6.4.1)) over the string `s`.
...
### [`string.gsub (s, pattern, repl [, n])`]{#pdf-string.gsub}

Returns a copy of `s` in which all (or the first `n`, if given)
occurrences of the `pattern` (see [§6.4.1](#6.4.1)) have been replaced
...
### [`string.lower (s)`]{#pdf-string.lower}
...
### [`string.match (s, pattern [, init])`]{#pdf-string.match}

Looks for the first *match* of `pattern` (see [§6.4.1](#6.4.1)) in the
string `s`. If it finds one, then `match` returns the captures from the
pattern; otherwise it returns **nil**. If `pattern` specifies no
...
### [`string.pack (fmt, v1, v2, ···)`]{#pdf-string.pack}
...
### [`string.rep (s, n [, sep])`]{#pdf-string.rep}
...
### [`string.reverse (s)`]{#pdf-string.reverse}

Returns a string that is the string `s` reversed.

------------------------------------------------------------------------

### [`string.sub (s, i [, j])`]{#pdf-string.sub}
...
### [`string.upper (s)`]{#pdf-string.upper}
...
-   a single character class followed by '`+`', which matches one or
    more repetitions of characters in the class. These repetition items
    will always match the longest possible sequence;
...
-   **`I[n]`:** an unsigned `int` with `n` bytes (default is native
    size)
...
-   **`s[n]`:** a string preceded by its length coded as an unsigned
    integer with `n` bytes (default is a `size_t`)
-   **`x`:** one byte of padding
-   **`Xop`:** an empty item that aligns according to option `op` (which
    is otherwise ignored)
-   **'` `':** (empty space) ignored

(A "`[n]`" means an optional integral numeral.) Except for padding,
spaces, and configurations (options "`xX <=>!`"), each option
corresponds to an argument (in [`string.pack`](#pdf-string.pack)) or a
result (in [`string.unpack`](#pdf-string.unpack)).

For options "`!n`", "`sn`", "`in`", and "`In`", `n` can be any integer
between 1 and 16. All integral options check overflows;
[`string.pack`](#pdf-string.pack) checks whether the given value fits in
the given size; [`string.unpack`](#pdf-string.unpack) checks whether the
...
must be a power of 2. Options "`c`" and "`z`" are not aligned; option
"`s`" follows the alignment of its starting integer.
...
"`[\0-\x7F\xC2-\xF4][\x80-\xBF]*`" (see [§6.4.1](#6.4.1)), which matches
exactly one UTF-8 byte sequence, assuming that the subject is a valid
UTF-8 string.

------------------------------------------------------------------------

### [`utf8.codes (s)`]{#pdf-utf8.codes}

Returns values so that the construction

         for p, c in utf8.codes(s) do body end

will iterate over all characters in string `s`, with `p` being the
position (in bytes) and `c` the code point of each character. It raises
an error if it meets any invalid byte sequence.

------------------------------------------------------------------------

### [`utf8.codepoint (s [, i [, j]])`]{#pdf-utf8.codepoint}
...
### [`utf8.offset (s, n [, i])`]{#pdf-utf8.offset}
...
### [`table.concat (list [, sep [, i [, j]]])`]{#pdf-table.concat}
...
### [`table.insert (list, [pos,] value)`]{#pdf-table.insert}
...
### [`table.move (a1, f, e, t [,a2])`]{#pdf-table.move}

Moves elements from table `a1` to table `a2`, performing the equivalent
...
### [`table.remove (list [, pos])`]{#pdf-table.remove}
...
### [`table.sort (list [, comp])`]{#pdf-table.sort}
...
the elements in the list; that is, it must be asymmetric and transitive.
...
### [`table.unpack (list [, i [, j]])`]{#pdf-table.unpack}
...
### [`math.abs (x)`]{#pdf-math.abs}

Returns the absolute value of `x`. (integer/float)

------------------------------------------------------------------------

### [`math.acos (x)`]{#pdf-math.acos}

Returns the arc cosine of `x` (in radians).

------------------------------------------------------------------------

### [`math.asin (x)`]{#pdf-math.asin}

Returns the arc sine of `x` (in radians).

------------------------------------------------------------------------

### [`math.atan (y [, x])`]{#pdf-math.atan}
...
### [`math.ceil (x)`]{#pdf-math.ceil}

Returns the smallest integral value larger than or equal to `x`.

------------------------------------------------------------------------

### [`math.cos (x)`]{#pdf-math.cos}

Returns the cosine of `x` (assumed to be in radians).

------------------------------------------------------------------------

### [`math.deg (x)`]{#pdf-math.deg}

Converts the angle `x` from radians to degrees.

------------------------------------------------------------------------

### [`math.exp (x)`]{#pdf-math.exp}

Returns the value *e^x^* (where `e` is the base of natural logarithms).

------------------------------------------------------------------------

### [`math.floor (x)`]{#pdf-math.floor}

Returns the largest integral value smaller than or equal to `x`.

------------------------------------------------------------------------

### [`math.fmod (x, y)`]{#pdf-math.fmod}
...
### [`math.rad (x)`]{#pdf-math.rad}

Converts the angle `x` from degrees to radians.

------------------------------------------------------------------------

### [`math.random ([m [, n]])`]{#pdf-math.random}
...
uniform distribution in the range *\[m, n\]*. (The value *n-m* cannot be
negative and must fit in a Lua integer.) The call `math.random(n)` is
equivalent to `math.random(1,n)`.
...
### [`math.randomseed (x)`]{#pdf-math.randomseed}
...
### [`math.sin (x)`]{#pdf-math.sin}

Returns the sine of `x` (assumed to be in radians).

------------------------------------------------------------------------

### [`math.sqrt (x)`]{#pdf-math.sqrt}
...
### [`math.tan (x)`]{#pdf-math.tan}

Returns the tangent of `x` (assumed to be in radians).

------------------------------------------------------------------------

### [`math.tointeger (x)`]{#pdf-math.tointeger}
...
### [`io.lines ([filename, ···])`]{#pdf-io.lines}
...
iterator function detects the end of file, it returns no values (to
...
### [`io.open (filename [, mode])`]{#pdf-io.open}
...
### [`io.popen (prog [, mode])`]{#pdf-io.popen}
...
subsequent formats.) When called without formats, it uses a default
...
-   **"`n`":** reads a numeral and returns it as a float or an integer,
    ...
    leading spaces and a sign.) This format always reads the longest
...
### [`file:seek ([whence [, offset]])`]{#pdf-file:seek}
...
### [`file:setvbuf (mode [, size])`]{#pdf-file:setvbuf}
...
### [`os.date ([format [, time]])`]{#pdf-os.date}
...
locale. (More specifically, `os.date()` is equivalent to
`os.date("%c")`.)
...
### [`os.execute ([command])`]{#pdf-os.execute}
...
### [`os.exit ([code [, close]])`]{#pdf-os.exit}
...
### [`os.getenv (varname)`]{#pdf-os.getenv}
...
### [`os.remove (filename)`]{#pdf-os.remove}
...
### [`os.rename (oldname, newname)`]{#pdf-os.rename}
...
### [`os.setlocale (locale [, category])`]{#pdf-os.setlocale}
...
For instance, if `sec` is -10, it means -10 seconds from the time
specified by the other fields; if `hour` is 1000, it means +1000 hours
from the time specified by the other fields.
...
### [`debug.gethook ([thread])`]{#pdf-debug.gethook}
...
### [`debug.getinfo ([thread,] f [, what])`]{#pdf-debug.getinfo}
...
### [`debug.getlocal ([thread,] f, local)`]{#pdf-debug.getlocal}
...
variables that are active in the current scope of the function. Negative
indices refer to vararg parameters; -1 is the first vararg parameter.
The function returns **nil** if there is no variable with the given
index, and raises an error when called with a level out of range. (You
can call [`debug.getinfo`](#pdf-debug.getinfo) to check whether the
level is valid.)
...
### [`debug.getmetatable (value)`]{#pdf-debug.getmetatable}
...
### [`debug.getupvalue (f, up)`]{#pdf-debug.getupvalue}
...
Variable names starting with '`(`' (open parenthesis) represent
variables with no known names (variables from chunks saved without debug
information).

------------------------------------------------------------------------

### [`debug.getuservalue (u)`]{#pdf-debug.getuservalue}

Returns the Lua value associated to `u`. If `u` is not a full userdata,
returns **nil**.

------------------------------------------------------------------------

### [`debug.sethook ([thread,] hook, mask [, count])`]{#pdf-debug.sethook}
...
any combination of the following characters, with the given meaning:
...
### [`debug.setlocal ([thread,] level, local, value)`]{#pdf-debug.setlocal}
...
### [`debug.setmetatable (value, table)`]{#pdf-debug.setmetatable}
...
### [`debug.setupvalue (f, up, value)`]{#pdf-debug.setupvalue}
...
### [`debug.setuservalue (udata, value)`]{#pdf-debug.setuservalue}
...
### [`debug.traceback ([thread,] [message [, level]])`]{#pdf-debug.traceback}
...
### [`debug.upvalueid (f, n)`]{#pdf-debug.upvalueid}
...
### [`debug.upvaluejoin (f1, n1, f2, n2)`]{#pdf-debug.upvaluejoin}
...
environment variable [`LUA_INIT_5_3`]{#pdf-LUA_INIT_5_3} (or
...
`@filename`, then `lua` executes the file. Otherwise, `lua` executes the
string itself.

When called with option `-E`, besides ignoring `LUA_INIT`, Lua also
ignores the values of `LUA_PATH` and `LUA_CPATH`, setting the values of
[`package.path`](#pdf-package.path) and
[`package.cpath`](#pdf-package.cpath) with the default paths defined in
`luaconf.h`.
...
will first set `a` to 1, then print the value of `a`, and finally run
the file `script.lua` with no arguments. (Here `$` is the shell prompt.
...
parameters `arg[1]`, ···, `arg[#arg]`. (Like all chunks in Lua, the
script is compiled as a vararg function.)
...
error object to a string and adds a stack traceback to it.
...
`#`. Therefore, Lua scripts can be made into executable programs by
...
program from Lua 5.2 to Lua 5.3. You can avoid some incompatibilities by
compiling Lua with appropriate options (see file `luaconf.h`). However,
...
should not assume that binaries are compatible between different Lua
...
8.1 – [Changes in the Language]{#8.1}
-------------------------------------

-   The main difference between Lua 5.2 and Lua 5.3 is the introduction
    of an integer subtype for numbers. Although this change should not
    affect "normal" computations, some computations (mainly those that
    involve some kind of overflow) can give different results.
    You can fix these differences by forcing a number to be a float (in
    Lua 5.2 all numbers were float), in particular writing constants
    with an ending `.0` or using `x = x + 0.0` to convert a variable.
    (This recommendation is only for a quick fix for an occasional
    incompatibility; it is not a general guideline for good programming.
    For good programming, use floats where you need floats and integers
    where you need integers.)
-   The conversion of a float to a string now adds a `.0` suffix to the
    result if it looks like an integer. (For instance, the float 2.0
    will be printed as `2.0`, not as `2`.) You should always use an
    explicit format when you need a specific format for numbers.
    (Formally this is not an incompatibility, because Lua does not
    specify how numbers are formatted as strings, but some programs
    assumed a specific format.)
-   The generational mode for the garbage collector was removed. (It was
    an experimental feature in Lua 5.2.)

8.2 – [Changes in the Libraries]{#8.2}
--------------------------------------

-   The `bit32` library has been deprecated. It is easy to require a
    compatible external library or, better yet, to replace its functions
    with appropriate bitwise operations. (Keep in mind that `bit32`
    operates on 32-bit integers, while the bitwise operators in Lua 5.3
    operate on Lua integers, which by default have 64 bits.)
-   The Table library now respects metamethods for setting and getting
    elements.
-   The [`ipairs`](#pdf-ipairs) iterator now respects metamethods and
    its `__ipairs` metamethod has been deprecated.
-   Option names in [`io.read`](#pdf-io.read) do not have a starting
    '`*`' anymore. For compatibility, Lua will continue to accept (and
    ignore) this character.
-   The following functions were deprecated in the mathematical library:
    `atan2`, `cosh`, `sinh`, `tanh`, `pow`, `frexp`, and `ldexp`. You
    can replace `math.pow(x,y)` with `x^y`; you can replace `math.atan2`
    with `math.atan`, which now accepts one or two parameters; you can
    replace `math.ldexp(x,exp)` with `x * 2.0^exp`. For the other
    operations, you can either use an external library or implement them
    in Lua.
-   The searcher for C loaders used by [`require`](#pdf-require) changed
    the way it handles versioned names. Now, the version should come
    after the module name (as is usual in most other tools). For
    compatibility, that searcher still tries the old format if it cannot
    find an open function according to the new style. (Lua 5.2 already
    worked that way, but it did not document the change.)
-   The call `collectgarbage("count")` now returns only one result. (You
    can compute that second result from the fractional part of the first
    result.)

8.3 – [Changes in the API]{#8.3}
--------------------------------

-   Continuation functions now receive as parameters what they needed to
    get through `lua_getctx`, so `lua_getctx` has been removed. Adapt
    your code accordingly.
-   Function [`lua_dump`](#lua_dump) has an extra parameter, `strip`.
...
-   Functions to inject/project unsigned integers (`lua_pushunsigned`,
    ...
    `luaL_optunsigned`) were deprecated. Use their signed equivalents
    with a type cast.
-   Macros to project non-default integer types (`luaL_checkint`,
    `luaL_optint`, `luaL_checklong`, `luaL_optlong`) were deprecated.
    Use their equivalent over [`lua_Integer`](#lua_Integer) with a type
    cast (or, when possible, use [`lua_Integer`](#lua_Integer) in your
    code).
